...
This file uses Baseline to train a Transformer model using fastBPE with query-response input using Seq2Seq
  
"""

def run_step(x, y, model, loss_function, distributed):
...
def parse_args(argv):
    ...
    parser.add_argument("--layer_norms_after", type=str2bool, default=False,
                        help="Layer norms after (set True for BERT)")
...
def run(basedir=None, train_file=None, valid_file=None, dataset_key='paired', embed_type='default',
    ...
    if restart_from:
        ...
        global_step, start_epoch = reload_from_checkpoint(restart_from, restart_tt, model,
                                                          steps_per_epoch)
...
def reload_from_checkpoint(restart_from, restart_tick_type, model, steps_per_epoch):
