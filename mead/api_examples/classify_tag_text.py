...
def main():
    ...
    m = bl.JointClassifyTaggerService.load(args.model, backend=args.backend, remote=args.remote,
    ...
    for texts in batched:
        for prediction in m.predict(texts, export_mapping=create_export_mapping(args.export_mapping)):
            sequence_prediction, class_string = prediction
            print(f"Class: {class_string}")
            for word_tag in sequence_prediction:
