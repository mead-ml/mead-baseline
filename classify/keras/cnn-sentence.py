...
from keras.layers import Dense, Activation, Convolution1D, Embedding, Input, merge, GlobalMaxPooling1D
...
def create(embeddings, nc, filtsz, cmotsz, hsz, maxlen, ft):
    x = Input(shape=(maxlen,), dtype='int32', name='input')

    vocab_size = embeddings.weights.shape[0]
    embedding_dim = embeddings.dsz
    
    lut = Embedding(input_dim=vocab_size, output_dim=embedding_dim, weights=[embeddings.weights], input_length=maxlen, trainable=ft)
    ...
    last_layer = joined
    if hsz > 0:
        proj = Dense(hsz, input_dim=cmotsz_all, activation='relu')(joined)
        input_dim = hsz
        last_layer = proj
...
parser.add_argument('--ft', help='Fine tune model', action='store_true')
...
vocab = buildVocab([args.train, args.test, args.valid], args.clean, args.chars, args.ft)
...
nc = len(f2i)
model = create(embeddings, nc, args.filtsz, args.cmotsz, args.hsz, args.mxlen)
loss_type = 'categorical_crossentropy'

model.compile(args.optim, loss_type, metrics=['accuracy'])
...
checkpoint = ModelCheckpoint('%s/cnn-sentence-fine.model' % args.outdir, monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto')
...
model = keras.models.load_model('%s/cnn-sentence-fine.model' % args.outdir)
