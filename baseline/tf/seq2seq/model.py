...
class EncoderDecoderModelBase(tf.keras.Model, EncoderDecoderModel):
    ...
    def create_test_loss(self):
        with tf.compat.v1.variable_scope('test_loss'):
            # We do not want to count <GO> in our assessment, we do want to count <EOS>
            return _temporal_cross_entropy_loss(self.decoder.preds[:-1, :, :],
                                                self.tgt_embedding.x[:, 1:],
                                                self.tgt_len - 1,
    ...
    def init_encoder(self, input_sz, **kwargs)  -> BaseLayer:
    ...
    def init_decoder(self, tgt_embedding, **kwargs) -> BaseLayer:
        return create_seq2seq_decoder(tgt_embedding, **kwargs)

    def encode(self, input: Dict[str, TensorDef], lengths: TensorDef) -> TensorDef:
        """
    ...
    def decode(self, encoder_outputs: TensorDef, dst: TensorDef) -> TensorDef:
    ...
    def create(cls, src_embeddings, tgt_embedding, **kwargs):
        ...
        if not tf.executing_eagerly():
            ...
            inputs['dst'] = tgt_embedding.create_placeholder('dst')
            ...
            encoder_output = model.encode(inputs, model.src_len)
            model.decode(encoder_output, inputs['dst'])
        return model

    def embed(self, input: Dict[str, TensorDef]) -> TensorDef:
