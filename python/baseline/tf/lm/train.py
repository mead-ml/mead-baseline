...
def fit_datasets(model_params, ts, vs, es=None, **kwargs):
    ...
    train_dataset = tf.data.Dataset.from_tensor_slices(to_tensors(ts, tgt_key))
    ...
    valid_dataset = tf.data.Dataset.from_tensor_slices(to_tensors(vs, tgt_key))
    valid_dataset = valid_dataset.batch(batchsz, drop_remainder=False)
    valid_dataset = valid_dataset.repeat(epochs + 1)
    valid_dataset = valid_dataset.prefetch(NUM_PREFETCH)

    test_dataset = tf.data.Dataset.from_tensor_slices(to_tensors(es, tgt_key))
