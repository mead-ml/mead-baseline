...
class WordPieceVectorizer1D(AbstractVectorizer):
    ...
    def __init__(self, **kwargs):
        ...
        if TOKENIZER is None:
            TOKENIZER = BertTokenizer.from_pretrained(handle)
        self.wordpiece_tok = TOKENIZER
...
class BERTEmbeddings(PyTorchEmbeddings):
    ...
    def __init__(self, name, **kwargs):
        ...
        if TOKENIZER is None:
            TOKENIZER = BertTokenizer.from_pretrained(kwargs.get('embed_file'))
        self.model = BertModel.from_pretrained(kwargs.get('embed_file'))
        self.vocab = TOKENIZER.vocab
        self.vsz = TOKENIZER.vocab # 30522 self.model.embeddings.word_embeddings.num_embeddings
